...
The TimingDetector subsystem implements 444 scintillator bars (3 columns x 148 rows) for precise timing measurements. It is the most detailed subsystem, built via GeoModelXML (`Gmx2Geo`) from an XML prototype file. Each bar is a `GeoFullPhysVol` registered as a sensitive volume through `SHiPTimingDetInterface`.

## Geometry Tree

```
Timing_Detector (Air, 5500×6500×500 mm)
 └─ 444 × scintillator bars (1680×60×10 mm each)
...
| Scintillator | 1.032 g/cm³  | Detector bars   |
...
- Verify bar count (444 = 3 x 148) matches FairShip
