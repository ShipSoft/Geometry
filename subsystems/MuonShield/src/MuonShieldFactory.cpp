...
GeoPhysVol* MuonShieldFactory::buildStation(const StationData& station) {
    ...
    auto* stationBox = new GeoBox(gm(station.containerHalfX), gm(station.containerHalfY),
    ...
    for (const PieceData& piece : station.pieces) {
        auto* pieceBox = new GeoBox(gm(piece.halfX), gm(piece.halfY), gm(piece.halfZ));
        ...
        GeoTrf::Transform3D trf = GeoTrf::Translate3D(gm(piece.centX), gm(piece.centY), 0.0);
...
GeoPhysVol* MuonShieldFactory::build() {
    ...
    auto* areaBox = new GeoBox(gm(s_areaHalfX), gm(s_areaHalfY), gm(s_areaHalfZ));
    ...
    for (const StationData& station : k_stations) {
        ...
        GeoTrf::Transform3D trf = GeoTrf::Translate3D(0.0, 0.0, gm(station.stationZ));
