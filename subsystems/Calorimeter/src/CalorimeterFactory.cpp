...
CalorimeterFactory::CalorimeterFactory(SHiPMaterials& materials,
...
double CalorimeterFactory::totalStackZ(const CalorimeterConfig& cfg) {
    ...
    for (int code : cfg.layers) {
        if      (code == 7)              total += cfg.lead_thickness_mm;
        else if (code >= 1 && code <= 4) total += cfg.scint_thickness_mm;
        else if (code == 5 || code == 6) total += cfg.hpl_thickness_mm;
        else if (code == 8)              total += cfg.airgap_mm;
        else throw std::runtime_error(
                "CalorimeterFactory: unknown layer code in 'layers': "
                + std::to_string(code));
    }
    total += cfg.gap_ecal_hcal_mm;
    for (int code : cfg.layers2) {
        if      (code == 7)              total += cfg.iron_thickness_mm;
        else if (code >= 1 && code <= 4) total += cfg.scint_thickness_mm;
        else if (code == 5 || code == 6) total += cfg.hpl_thickness_mm;
        else if (code == 8)              total += cfg.airgap_mm;
        else throw std::runtime_error(
                "CalorimeterFactory: unknown layer code in 'layers2': "
                + std::to_string(code));
    }
    return total;
}
...
GeoPhysVol* CalorimeterFactory::build() {
    ...
    const double pitchX =
        cfg.module_pitch_x_mm > 0.0 ? cfg.module_pitch_x_mm : cfg.plate_xy_mm;
    const double pitchY =
        cfg.module_pitch_y_mm > 0.0 ? cfg.module_pitch_y_mm : cfg.plate_xy_mm;
    ...
    for (int iy = 0; iy < cfg.module_ny; ++iy)
        for (int ix = 0; ix < cfg.module_nx; ++ix)
            buildStack(containerPhys, cfg, ix, iy,
                       x0 + ix * pitchX,
                       y0 + iy * pitchY);
...
void CalorimeterFactory::buildStack(GeoPhysVol*              container,
                                    ...
                                    double                   offY) const
{
    ...
    auto makeEnv = [&](const std::string& name,
                       double             halfZ,
    ...
    for (int code : cfg.layers) {
        ...
        if (code == 7) {
            ...
            const std::string n = "/SHiP/calorimeter/ecal/gl" + std::to_string(gl)
                + "_sl" + std::to_string(sl) + "_wide_pvt_h" + mtag;
            auto* env = makeEnv(n, 0.5*scintZ, zCursor + 0.5*scintZ);
            CaloBarLayer::place(env, wideHLog, 60.0, 36, 0.0, n.c_str(),
                iWideH, BarAxis::AlongY, mtag);
            zCursor += scintZ; ++iWideH; ++gl; ++sl;

        } else if (code == 2) {
            const std::string n = "/SHiP/calorimeter/ecal/gl" + std::to_string(gl)
                + "_sl" + std::to_string(sl) + "_wide_pvt_v" + mtag;
            auto* env = makeEnv(n, 0.5*scintZ, zCursor + 0.5*scintZ);
            CaloBarLayer::place(env, wideVLog, 60.0, 36, 0.0, n.c_str(),
                iWideV, BarAxis::AlongX, mtag);
            zCursor += scintZ; ++iWideV; ++gl; ++sl;

        } else if (code == 3) {
            const std::string n = "/SHiP/calorimeter/ecal/gl" + std::to_string(gl)
                + "_sl" + std::to_string(sl) + "_thin_ps_h" + mtag;
            auto* env = makeEnv(n, 0.5*scintZ, zCursor + 0.5*scintZ);
            CaloBarLayer::place(env, thinHLog, 10.0, 216, 0.0, n.c_str(),
                iThinH, BarAxis::AlongY, mtag);
            zCursor += scintZ; ++iThinH; ++gl; ++sl;

        } else if (code == 4) {
            const std::string n = "/SHiP/calorimeter/ecal/gl" + std::to_string(gl)
                + "_sl" + std::to_string(sl) + "_thin_ps_v" + mtag;
            auto* env = makeEnv(n, 0.5*scintZ, zCursor + 0.5*scintZ);
            CaloBarLayer::place(env, thinVLog, 10.0, 216, 0.0, n.c_str(),
                iThinV, BarAxis::AlongX, mtag);
            zCursor += scintZ; ++iThinV; ++gl; ++sl;

        } else if (code == 5) {
            const std::string n = "/SHiP/calorimeter/ecal/gl" + std::to_string(gl)
                + "_sl" + std::to_string(sl) + "_hpl_y" + mtag;
            auto* env = makeEnv(n, 0.5*hplZ, zCursor + 0.5*hplZ);
            CaloFibreHPLayer::build(env, alMat, psMat, n, 0.0, iHPL,
                cfg.plate_xy_mm, cfg.hpl_thickness_mm,
                cfg.fiber_diameter_mm, cfg.fiber_core_diameter_mm, true, mtag);
            zCursor += hplZ; ++iHPL; ++gl; ++sl;

        } else if (code == 6) {
            const std::string n = "/SHiP/calorimeter/ecal/gl" + std::to_string(gl)
                + "_sl" + std::to_string(sl) + "_hpl_x" + mtag;
            auto* env = makeEnv(n, 0.5*hplZ, zCursor + 0.5*hplZ);
            CaloFibreHPLayer::build(env, alMat, psMat, n, 0.0, iHPL,
                cfg.plate_xy_mm, cfg.hpl_thickness_mm,
                cfg.fiber_diameter_mm, cfg.fiber_core_diameter_mm, false, mtag);
            zCursor += hplZ; ++iHPL; ++gl; ++sl;
    ...
    for (int code : cfg.layers2) {
        ...
        if (code == 7) {
            const std::string tag = "/SHiP/calorimeter/hcal/gl"
                + std::to_string(gl) + "_iron_" + std::to_string(iIron) + mtag;
            container->add(new GeoNameTag(tag.c_str()));
            container->add(new GeoIdentifierTag(layerId++));
            container->add(new GeoTransform(
                GeoTrf::Translate3D(offX, offY, zCursor + 0.5*ironZ)));
            container->add(new GeoPhysVol(ironLog));
            zCursor += ironZ; ++iIron; ++gl;

        } else if (code == 1) {
            const std::string n = "/SHiP/calorimeter/hcal/gl" + std::to_string(gl)
                + "_sl" + std::to_string(sl) + "_wide_pvt_h" + mtag;
            auto* env = makeEnv(n, 0.5*scintZ, zCursor + 0.5*scintZ);
            CaloBarLayer::place(env, wideHLog, 60.0, 36, 0.0, n.c_str(),
                iWideH, BarAxis::AlongY, mtag);
