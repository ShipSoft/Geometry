...
// ── file-scope geometry parameters and helpers ───────────────────────────────
namespace {

constexpr const char* kBase = "/SHiP/neutrino_detector";

// Veto: upstream PVT bar station (3 planes × 7 bars).
constexpr auto s_veto_bar_length = 420.0 * mm;
constexpr auto s_veto_bar_width = 60.0 * mm;
constexpr auto s_veto_bar_thickness = 10.0 * mm;
constexpr int s_veto_n_bars = 7;
constexpr auto s_veto_shift_y = 9.0 * mm;    // ±stagger of planes 0/1 in Y
constexpr auto s_veto_plane_gap = 5.0 * mm;  // air gap between plane faces
constexpr auto s_veto_gap_to_target = 20.0 * mm;
constexpr auto s_veto_plane_xy = s_veto_n_bars * s_veto_bar_width;  // 420 mm
constexpr auto s_veto_total_thickness =
    3.0 * s_veto_bar_thickness + 2.0 * s_veto_plane_gap;  // 40 mm

// NuTarget: silicon/tungsten sampling target (120 layers).
constexpr auto s_tgt_plate_xy = 400.0 * mm;
constexpr auto s_tgt_w = 3.5 * mm;
constexpr auto s_tgt_si = 0.300 * mm;
constexpr auto s_tgt_w_si_gap = 0.200 * mm;
constexpr auto s_tgt_si_si_gap = 6.800 * mm;
constexpr int s_tgt_n_layers = 120;
constexpr auto s_tgt_pitch =
    s_tgt_w + s_tgt_w_si_gap + s_tgt_si + s_tgt_si_si_gap + s_tgt_si + s_tgt_w_si_gap;  // 11.3 mm

// HCAL: 3 transverse sections, 14 layers each.
constexpr auto s_hcal_gap_to_target = 10.0 * mm;
constexpr auto s_hcal_iron = 50.0 * mm;
constexpr auto s_hcal_fibre_diameter = 0.250 * mm;  // round scintillating fibre
constexpr int s_hcal_fibre_sublayers = 2;           // staggered sub-layers per plane
constexpr auto s_hcal_fibre_plane =
    s_hcal_fibre_sublayers * s_hcal_fibre_diameter;  // 0.5 mm fibre-plane thickness
constexpr auto s_hcal_tile_xy = 50.0 * mm;
constexpr auto s_hcal_tile_thickness = 10.0 * mm;
constexpr auto s_hcal_layer =
    s_hcal_iron + 2.0 * s_hcal_fibre_plane + s_hcal_tile_thickness;  // 61 mm
...
void placeChild(GeoVPhysVol* mother, const GeoLogVol* log, const std::string& name, int id,
                LengthMm x, LengthMm y, LengthMm z) {
    mother->add(new GeoNameTag(name));
    mother->add(new GeoIdentifierTag(id));
    mother->add(new GeoTransform(GeoTrf::Translate3D(gm(x), gm(y), gm(z))));
    mother->add(new GeoPhysVol(log));
}

/// Build the upstream PVT veto: three planes of seven bars, laid out working
/// upstream from @p zDownstreamFace (planes 0/1 horizontal and staggered in
/// Y, plane 2 vertical).
void buildVeto(GeoVPhysVol* mother, const GeoMaterial* pvt, LengthMm zDownstreamFace) {
    const double halfL = 0.5 * gm(s_veto_bar_length);
    const double halfW = 0.5 * gm(s_veto_bar_width);
    const double halfT = 0.5 * gm(s_veto_bar_thickness);
    ...
    const LengthMm zc[3] = {zDownstreamFace - 2.5 * s_veto_bar_thickness - 2.0 * s_veto_plane_gap,
                            zDownstreamFace - 1.5 * s_veto_bar_thickness - 1.0 * s_veto_plane_gap,
                            zDownstreamFace - 0.5 * s_veto_bar_thickness};
    const LengthMm yShift[3] = {+s_veto_shift_y, -s_veto_shift_y, 0.0 * mm};

    for (int p = 0; p < 3; ++p) {
        const bool vertical = (p == 2);
        for (int b = 0; b < s_veto_n_bars; ++b) {
            const LengthMm stack =
                -0.5 * s_veto_plane_xy + 0.5 * s_veto_bar_width + b * s_veto_bar_width;
            const LengthMm x = vertical ? stack : 0.0 * mm;
            const LengthMm y = vertical ? 0.0 * mm : stack + yShift[p];
...
void buildTarget(GeoVPhysVol* mother, const GeoMaterial* tungsten, const GeoMaterial* silicon,
                 LengthMm zStart) {
    const double halfXY = 0.5 * gm(s_tgt_plate_xy);
    auto* wLog = new GeoLogVol(std::string(kBase) + "/target/W_plate",
                               new GeoBox(halfXY, halfXY, 0.5 * gm(s_tgt_w)), tungsten);
    auto* siXLog = new GeoLogVol(std::string(kBase) + "/target/Si_X",
                                 new GeoBox(halfXY, halfXY, 0.5 * gm(s_tgt_si)), silicon);
    ...
    for (int l = 0; l < s_tgt_n_layers; ++l) {
        ...
        placeChild(mother, wLog, tag + "/W", l, 0.0 * mm, 0.0 * mm, z + 0.5 * s_tgt_w);
        z += s_tgt_w + s_tgt_w_si_gap;
        placeChild(mother, siXLog, tag + "/Si_X", l, 0.0 * mm, 0.0 * mm, z + 0.5 * s_tgt_si);
        z += s_tgt_si + s_tgt_si_si_gap;
        placeChild(mother, siYLog, tag + "/Si_Y", l, 0.0 * mm, 0.0 * mm, z + 0.5 * s_tgt_si);
...
/// physical volume; @p zFront is the plane's upstream face.
void buildFibrePlane(GeoVPhysVol* mother, const GeoLogVol* envLog, GeoPhysVol* fibrePhys, bool isX,
                     LengthMm sectionXY, LengthMm zFront, const std::string& lTag) {
    const LengthMm r = 0.5 * s_hcal_fibre_diameter;
    const LengthMm d = s_hcal_fibre_diameter;
    const LengthMm halfXY = 0.5 * sectionXY;
    const LengthMm envHalfZ = s_hcal_fibre_sublayers * r;  // 0.25 mm

    const std::string envName = lTag + (isX ? "/FibreX" : "/FibreY");
    auto* envPhys = new GeoPhysVol(envLog);
    mother->add(new GeoNameTag(envName));
    mother->add(new GeoTransform(GeoTrf::Translate3D(0.0, 0.0, gm(zFront + envHalfZ))));
    mother->add(envPhys);

    // Tube axis is Z; rotate it to lie along X (X plane) or Y (Y plane).
    const GeoTrf::Transform3D rot =
        isX ? GeoTrf::Transform3D(GeoTrf::RotateY3D(gm(-90.0 * units::deg)))  // Z → X
            : GeoTrf::Transform3D(GeoTrf::RotateX3D(gm(90.0 * units::deg)));  // Z → Y

    const int nFibres = static_cast<int>(units::ratio(2.0 * halfXY / d));
    ...
    // (Y for an X plane, X for a Y plane); Pow(step, i) advances by i·d. The
    // genfunction owns its own double math (rule: doubles through GeoGenfun),
    // so the fibre positions convert to GeoModel units up front.
    ...
    const GeoTrf::Transform3D unitStep =
        isX ? GeoTrf::Transform3D(GeoTrf::Translate3D(0.0, gm(1.0 * mm), 0.0))
            : GeoTrf::Transform3D(GeoTrf::Translate3D(gm(1.0 * mm), 0.0, 0.0));

    for (int sub = 0; sub < s_hcal_fibre_sublayers; ++sub) {
        const LengthMm localZ = -envHalfZ + r + sub * d;
        const LengthMm stagger = (sub == 1) ? r : 0.0 * mm;
        const double start = gm(-halfXY + r + stagger);

        GeoGenfun::Variable i;
        GeoGenfun::GENFUNCTION pos = start + gm(d) * i;  // fibre centre along measuring axis
        GeoXF::TRANSFUNCTION fibreXF =
            GeoTrf::Transform3D(GeoTrf::Translate3D(0.0, 0.0, gm(localZ))) *
...
void buildHCal(GeoVPhysVol* mother, const GeoMaterial* air, const GeoMaterial* iron,
               const GeoMaterial* polystyrene, LengthMm zStart) {
    // Tile shape is section-independent → one shared log volume.
    auto* tileLog = new GeoLogVol(std::string(kBase) + "/hcal/tile",
                                  new GeoBox(0.5 * gm(s_hcal_tile_xy), 0.5 * gm(s_hcal_tile_xy),
                                             0.5 * gm(s_hcal_tile_thickness)),
                                  polystyrene);

    const LengthMm r = 0.5 * s_hcal_fibre_diameter;
    const LengthMm envHalfZ = s_hcal_fibre_sublayers * r;

    LengthMm z = zStart + s_hcal_gap_to_target;
    int tileId = 0;

    for (int s = 0; s < s_hcal_n_sections; ++s) {
        const LengthMm sectionXY = s_hcal_section_xy[s];
        const double halfXY = 0.5 * gm(sectionXY);
        // Fibre-plane air envelope is one fibre diameter larger than the section
        // so the staggered sub-layer is fully contained.
        const double envHalfXY = gm(0.5 * sectionXY + s_hcal_fibre_diameter);
        const std::string sTag = std::string(kBase) + "/hcal/S" + std::to_string(s);

        // Per-section shared log volumes (reused across all 14 layers).
        auto* ironLog =
            new GeoLogVol(sTag + "/Fe", new GeoBox(halfXY, halfXY, 0.5 * gm(s_hcal_iron)), iron);
        auto* fibreEnvLog =
            new GeoLogVol(sTag + "/Fibre_ENV", new GeoBox(envHalfXY, envHalfXY, gm(envHalfZ)), air);
        auto* fibreLog =
            new GeoLogVol(sTag + "/fibre", new GeoTube(0.0, gm(r), halfXY), polystyrene);
        // One shared fibre physical volume placed by every plane in this section.
        auto* fibrePhys = new GeoPhysVol(fibreLog);

        const int nTiles = static_cast<int>(std::lround(units::ratio(sectionXY / s_hcal_tile_xy)));
        const LengthMm tileHalf = 0.5 * s_hcal_tile_xy;

        for (int l = 0; l < s_hcal_n_layers; ++l) {
            const std::string lTag = sTag + "_L" + std::to_string(l);

            placeChild(mother, ironLog, lTag + "/Fe", l, 0.0 * mm, 0.0 * mm, z + 0.5 * s_hcal_iron);
            z += s_hcal_iron;
            ...
            const LengthMm tileZc = z + 0.5 * s_hcal_tile_thickness;
            for (int row = 0; row < nTiles; ++row) {
                const LengthMm y = -0.5 * sectionXY + tileHalf + row * s_hcal_tile_xy;
                for (int col = 0; col < nTiles; ++col) {
                    const LengthMm x = -0.5 * sectionXY + tileHalf + col * s_hcal_tile_xy;
                    const std::string name =
